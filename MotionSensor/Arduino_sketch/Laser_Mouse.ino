...
void loop() 
{
  if (StartPolling == true)
  {
    ...
    xydat[0] = (byte)adns_read_reg(slave2, REG_Delta_X_L);
    xydat[1] = (byte)adns_read_reg(slave2, REG_Delta_X_H);    
    xydat[2] = (byte)adns_read_reg(slave2, REG_Delta_Y_L);
    xydat[3] = (byte)adns_read_reg(slave2, REG_Delta_Y_H);
    ...
    while(Serial.available())
    {
      ...
      switch (rx)
      {
        case 0x01:  // Sensor initialization requested from host
           ...
           performStartup(slave2);
           //DisplayRegisters(slave2);
           delay(100);
           Serial.write(0x01);
           break;        
        ...
        case 0x03:  // Get DPI and LIFT DISTANCE request from host
           ...
           Serial.write(rx);
           rx = adns_read_reg(slave2, REG_Configuration_I);
           Serial.write(rx);
           rx = adns_read_reg(slave1, REG_Lift_Detection_Thr);
           Serial.write(rx);
           rx = adns_read_reg(slave2, REG_Lift_Detection_Thr);
           Serial.write(rx);
           break;
        ...
        case 0x04: // Host requests to set sensor DPI and LIFT DISTANCE
           ...
           rx = Serial.read();
           adns_write_reg(slave2, REG_Configuration_I, rx);
           rx = Serial.read();
           adns_write_reg(slave1, REG_Lift_Detection_Thr, rx);
           rx = Serial.read();
           adns_write_reg(slave2, REG_Lift_Detection_Thr, rx);
           break;
